...
function WordNode({ label, state, isSelected, onClick }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      style={{ fontSize: 11, padding: '5px 9px', borderRadius: 5, cursor: onClick ? 'pointer' : 'default', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%', transition: 'filter 0.1s', ...base, ...selectedExtra }}
    >
      {label}
    </div>
  )
