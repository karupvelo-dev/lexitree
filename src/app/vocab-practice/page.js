...
function selectWords(allWords, vocabularySeen) {
  if (!vocabularySeen || Object.keys(vocabularySeen).length === 0) {
    return shuffle(allWords).slice(0, TOTAL)
  }
  return [...allWords]
...
export default function VocabPracticePage() {
  ...
  useEffect(() => {
    const lvl = localStorage.getItem('lexitree_level')
    ...
    if (user) {
      supabaseBrowser
        .from('profiles')
        .select('vocabulary_seen, vocabulary_correct')
        .eq('id', user.id)
        .single()
        .then(({ data }) => {
          ...
          setQuestions(buildQuestions(selectWords(words, seen), words))
        })
        .catch(() => setQuestions(buildQuestions(selectWords(words, null), words)))
    } else {
  ...
  async function saveProgress() {
    try {
      ...
      await supabaseBrowser
        .from('profiles')
        .update({ vocabulary_seen: seen, vocabulary_correct: correct, vocab_score: newScore, vocab_words_seen: newWordsSeenCount })
        .eq('id', user.id)
  ...
  return (
    <div className="app-shell">
      <Sidebar active="vocab-practice" user={user} signInWithGoogle={signInWithGoogle} signOut={signOut} vocabScoreOverride={scoreAfter} vocabWordsSeenOverride={wordsSeenCount} />

      <div className="main-content">
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '12px' }}>
          <h1 className="page-title" style={{ marginBottom: 0 }}>7 words</h1>
          <span style={{ fontSize: '13px', color: 'var(--light)' }}>Least-practised first</span>
        </div>
